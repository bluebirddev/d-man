...
import { RestApiExecutor, RequestOptions, rest } from './rest';

/**
 * simple api executor that always returns 200, and the same data that was passed in.
 */
const simpleApiExecutor: RestApiExecutor = (requestOptions) => {
...
describe('execute', () => {
    ...
    it('perform before request', async () => {
        ...
        let state = 1;
        const restResponsePromise = await rest(
            1,
            {
                ...requestOptions,
                beforeExecute: ({ data }) => {
    ...
    it('perform transform response data', async () => {
        const restResponse = await rest<number>(
            1,
            {
                ...simpleRequestOptions,
